...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/17 13:53:55 by flhember          #+#    #+#             */
/*   Updated: 2019/06/20 17:43:10 by flhember         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_calc_z(t_thr *thr)
{
	...
	thr->data.pts.z_i = (2 * thr->data.pts.z_i * thr->data.pts.tmp)
					+ thr->data.pts.c_i;
	thr->data.pts.z_r < 0 ? thr->data.pts.z_r *= -1 : thr->data.pts.z_r;
	thr->data.pts.z_i < 0 ? thr->data.pts.z_i *= -1 : thr->data.pts.z_i;
}
...
static void	ft_calc_pixel(t_thr *thr)
{
	...
	ft_color_deg(&thr->data, thr->data.pts.x - thr->data.pts.trans_x
		, thr->data.pts.y - thr->data.pts.trans_y, i);
}

void		*ft_calc_burning_ship(void *thd)
{
	t_thr	*thr;
